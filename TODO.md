...
- [ ] Skip sidechain / subagent threads (`isSidechain: true` in Claude) — those are agent-to-agent, not the user.
- [ ] Skip hook-injected user messages (e.g. `[_HIDE_TEST_]`, `<system-reminder>`) — they're not the user.

## 3. Identifying user messages
...
- [ ] Add a `is_user_message(turn) -> bool` predicate per harness; unit-test on a handful of known sessions.
...
Implemented in `user_mimic/filters.py::classify_first_user_message` + thread-level check in `loader.py::samples_from_thread`. Per-signal drop counts logged.

## 4. Sample builder (`template.py`)
- [ ] Emit canonical sample dict: `{context_messages: [...], target: "<user msg>", project: "<slug>", harness, model}`. Both tracks consume this.
...
## 5. System prompt
- [ ] Template: `This is a log from model {model} in harness {harness}, cwd {cwd}. Respond as the user.`
...
- [ ] Strip email addresses except the user's own (optional).
...
## 7. Filtering / balance
- [ ] Drop user messages of length < 2 tokens ("ok", "y") — they'll dominate and teach nothing.
- [ ] Optionally drop trivial one-word confirmations above some frequency cap.
- [ ] De-dup near-identical user messages across threads (hash-based).
- [ ] Date filter: only include sessions from last N months so the model learns the *current* user style.

## 8. Splits & eval
- [ ] Hold out ~5% of threads (not messages) for eval — prevents leakage.
- [ ] Eval metric: loss on held-out user messages + a few qualitative generations.
- [ ] Write `eval.py` that samples completions given a real prior context.

## 9a. Track A — Metric (`metric.py`)
- [ ] Implement LLM-as-judge: pairwise "which reply is more user-like given this thread?" between (generated, gold). Use a cheap judge (Haiku / GPT-5-mini).
- [ ] Implement embedding cosine (sentence-transformers or cloud embed API) between generated and gold.
- [ ] Length sanity bonus/penalty — the user is terse; reward being within 0.5×–2× gold length.
- [ ] Combined score = weighted sum. Start with weights (0.6 judge, 0.3 embed, 0.1 length) and tune.
- [ ] Sanity-check metric manually: does it rank obviously-user replies above generic ones?
...
- [ ] Stage 2: for each project with ≥ N (e.g. 30) user msgs, freeze PREFIX, optimize a short SUFFIX on that project's train split. Project split defined by `cwd` → repo-root slug.
...
- [ ] Decide the shape: standalone CLI that watches a live Claude Code jsonl and suggests user-style replies? A wrapper harness? A VS Code thing? — **pick one before v2.**
...
- [ ] Include `sidechain` subagent threads? (Leaning: no — the user isn't the user there.)
- [ ] Should system prompt also include the *current* user memory (`~/.claude/.../memory/*.md`)? Could be strong steering signal.
